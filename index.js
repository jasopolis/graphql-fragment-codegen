...
const fragmentNameSuffix = 'AllFields'
...
function makeFragments(schemaContents) {
    ...
    // Create fragments based on all objects in schema
    const fragments = types
        ...
        .map(
            ({ name, fields }) =>
                `fragment ${name}${fragmentNameSuffix} on ${name}` +
                ' {' +
                indentedLine(1) +
                fields
...
function printField(field, typeLookupTable, indent = 1) {
    ...
    if (type.kind === 'OBJECT') {
        ...
        return name + ' { ' + '...' + type.name + 'AllFields' + ' }'
    }
    if (type.kind === 'UNION' || type.kind === 'INTERFACE') {
        ...
        return (
            ...
            '__typename' +
            indentedLine(indent + 1) +
            // Iterate through possible (non-union) types in union type
            // and include all fields within them
